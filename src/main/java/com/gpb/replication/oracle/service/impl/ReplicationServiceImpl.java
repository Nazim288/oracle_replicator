...
public class ReplicationServiceImpl implements ReplicationService {
                ...
                try {
                    ...
                    for (String schema : schemas) {
                        try {
                            ...
                            log.error("Ошибка в tableReplicationOracle(db={}, schema={})", dbName, schema, e);
                        }
                    }
    ...
    private void saveTablesInBatches(List<TableMetadata> entities) {
        int batchSize = 500;

        for (int i = 0; i < entities.size(); i += batchSize) {
            int end = Math.min(i + batchSize, entities.size());
            tableRep.saveAll(entities.subList(i, end));
            tableRep.flush();

            log.info("Saved table batch: {} - {} of {}", i, end, entities.size());
        }
    }
}
